        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Class Room</h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
                    <a href="{{ url('admin/hall') }}" class="btn btn-primary m-btn m-btn--icon"><i class="far fa-building pr-2"></i>Class Rooms</a>
            </div>
        </div>

        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right"  action="{{ url('admin/hall/' .$hall->id) }}" method="post">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('title') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="title" value="{{ old('title', $hall->title) }}" placeholder="Room Title"/>
                            @if ($errors->has('title'))
...
{{--                                    <option value="{{$key}}" {{$key == $hall->floor_number ? 'selected' : ' '}}>{{$floor}}</option>--}}
{{--                                @endforeach--}}
{{--                            </select>--}}
...
{{--                    </div>--}}
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('floor_number') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="floor_number" value="{{ old('floor_number', $hall->floor_number) }}" placeholder="Floor"/>
                            @if ($errors->has('floor_number'))
                                <div class="form-control-feedback"> {{ $errors->first('floor_number') }} </div>
                            @endif
                        </div>
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('room_number') ? 'has-danger' : '' }}">
                            ...
                            <input type="number" class="form-control m-input" name="room_number" value="{{ old('room_number', $hall->room_number) }}" placeholder="Room Number"/>
            ...
            <div class="m-portlet__foot m-portlet__foot--fit">
                <div class="m-form__actions text-center">
                    <a href="{{ url('admin/hall') }}" class="btn btn-outline-brand"><i class="fa fa-times"></i> Cancel</a>
