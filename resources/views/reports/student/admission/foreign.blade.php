        ...
        <div class="col-md-12">
            <div class="m-portlet">
                ...
                <div class="m-portlet__body">
                    ...
                    <form id="searchForm" role="form" method="get" action="{{route('report.student.admission.type', [2])}}">
                        <div class="form-body">
                            <div class="row">
                                        ...
                                        <div class="btn-group" data-hover="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right w-100">
                                                <a class="dropdown-item"
                                                   href="{{ route('report.student.admission.print.type', array_merge([2, 'session_id' => $sessionId, 'course_id' => $courseId], ['page_layout' => 'A4-landscape'])) }}"
                                                   target="_blank">
                                                    <i class="fas fa-arrows-alt-h"></i> Landscape
                                                </a>
                                                <a class="dropdown-item"
                                                   href="{{ route('report.student.admission.print.type', array_merge([2, 'session_id' => $sessionId, 'course_id' => $courseId], ['page_layout' => 'A4-portrait'])) }}"
                                                   target="_blank">
                                                    <i class="fas fa-arrows-alt-v"></i> Portrait
                                                </a>
                                            </div>
                                        </div>
                                       <a class="btn btn-primary m-btn m-btn--icon" href="{{route('report.student.admission.excel.type', [2, 'session_id' => $sessionId, 'course_id' => $courseId])}}" class="btn btn-brand m-btn m-btn--icon"><i class="fa fa-file-export"></i> Export</a>
                                        @endif
                                        <button class="btn btn-primary m-btn m-btn--icon"><i class="fa fa-search"></i> Search</button>
                                        <a href="{{route('report.student.admission.type', [2])}}" class="btn btn-default reset"><i class="fas fa-sync-alt search-reset"></i> Reset</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                                            ...
                                            @if(isset($searchResult))
                                                @if(!empty($searchResult))
                                                    ...
                                                    @foreach($searchResult as $student)
                                                        ...
                                                        <tr>
                                                            <td>{{$i}}</td>
                                                            <td>{!! '<b>Name:</b> '.$student->full_name_en.'<br> <b>Father:</b> '.$student->parent->father_name.'<br> <b>Mother:</b> '.$student->parent->mother_name !!}</td>
