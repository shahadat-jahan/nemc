                                            ...
                                            <div class="btn-group" data-hover="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right w-100">
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.exam_result.category.pdf', array_merge(request()->all(), ['page_layout' => 'A4-landscape'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-h"></i> Landscape
                                                    </a>
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.exam_result.category.pdf', array_merge(request()->all(), ['page_layout' => 'A4-portrait'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-v"></i> Portrait
                                                    </a>
                                                </div>
                                            </div>
                                            <a href="{{route('report.exam_result.category.excel', [
    ...
    <script>
        ...
        $('#session_id, #course_id, #phase_id, #term_id, #subject_id, #exam_category_id').change(function () {
            ...
            if (sessionId > 0 && courseId > 0 && phaseId > 0 && termId > 0 && subjectId > 0 && examCategoryId > 0) {
                ...
                // load exam
                $.get('{{route('exam.list.session.course.phase.term.subject')}}', {
                    sessionId: sessionId,
                    courseId: courseId,
                    phaseId: phaseId,
                    termId: termId,
