                        ...
                        <div class="row mb-3">
                            <div class="col-xs-12 col-sm-6 col-md-12 col-lg-12 col-xl-12">
                                <div class="pull-right search-action-btn">
                                    ...
                                    <a  class="btn btn-default m-btn m-btn--icon" href="{{url('admin/report_teacher_wise_class')}}"><i class="fa fa-search"></i> Reset</a>
                                </div>
                            </div>
                        </div>
...
<!--                        <div class="row mb-3">
                       ...
                       @if(isset($teacherWiseClass))
                        <div class="row">
                            <div class="col-md-12">
                                <h4 class="text-center">Teacher Wise Class</h4>
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col">Get Class</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                         @if(!empty($teacherWiseClass))
                                         @php $sl=1; @endphp
                                         @foreach($teacherWiseClass as $key=>$value)
                                          <tr>
                                            ...
                                            <td rowspan="2">
                                                @php
                                                $teacher_id=!empty($value['teacher_id']) ? $value['teacher_id'] :' ';
                                                $first_name=!empty($value['first_name']) ? $value['first_name'] :' ';
                                                $last_name=!empty($value['last_name']) ? $value['last_name'] :' ';
                                                $full_name=$first_name.' '.$last_name
                                                @endphp
                                                <!--<a href="{{ url('admin/teacher/'.$teacher_id) }}" target="_blank">-->
                                                {{!empty($full_name) ? $full_name :'---'}}
                                                <!--</a>-->
                                            </td>
                                            <td rowspan="2">{{$value['total_class']}}</td>
                                            <td rowspan="2">
                                                {{getTotalClass($teacher_id,$sessionId,$phaseId,$termId,$subjectId,false,false,$fromDate,$toDate,$courseId)}}
                                            </td>
                                          <tr/>
                                         @php $sl++; @endphp
                                         @endforeach
