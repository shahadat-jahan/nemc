            ...
            <div class="m-portlet">
                ...
                <div class="m-portlet__body">
                    <div class="m-section__content">
                        <form role="form" id="nemc-general-form" method="get">
                            <div class="row">
                                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                    <div class="form-group">
                                        <select class="form-control" name="session_id" id="session_id">
                                            <option value="">---- Select Session ----</option>
                                            {!! select($sessions, request('session_id')) !!}
                                        </select>
                                    </div>
                                </div>
                                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                    <div class="form-group">
                                        <select class="form-control" name="course_id" id="course_id">
                                            ...
                                            {!! select($courses, Auth::user()->teacher->course_id ?? request('course_id')) !!}
                                        </select>
                                    </div>
                                </div>
                                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                    <div class="form-group">
                                        <select class="form-control" name="phase_id" id="phase_id">
                                            ...
                                            {!! select($phases, request('phase_id')) !!}
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row my-3">
                                <div class="col-xs-12 col-sm-6 col-md-12 col-lg-12 col-xl-12">
                                    <div class="pull-right search-action-btn">
                                        @if(!empty($subjects) && !empty($attendanceData))
                                            <div class="btn-group" data-hover="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right w-100">
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.attendance.phase.pdf', array_merge(request()->all(), ['page_layout' => 'A4-landscape'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-h"></i> Landscape
                                                    </a>
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.attendance.phase.pdf', array_merge(request()->all(), ['page_layout' => 'A4-portrait'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-v"></i> Portrait
                                                    </a>
                                                </div>
                                            </div>
                                            <a href="{{route('report.attendance.phase.excel', request()->all())}}"
                                        ...
                                        <a class="btn btn-default m-btn m-btn--icon"
                                           href="{{url('admin/report_attendance_by_phase')}}">
                                ...
                                <div class="col-md-12">
                                    <div class="m-table table-responsive">
                                        <table class="table table-bordered table-hover sticky-header">
                                            <thead>
                                            <tr>
                                                <th rowspan="3" class="align-middle sticky">Roll No</th>
                                                <th rowspan="3" class="text-center align-middle sticky sticky-2">Student Name</th>
                                            ...
                                            @foreach(($attendanceByStudent ?? collect()) as $rollNo => $studentInfo)
                                                ...
                                                @endphp
                                                <tr>
                                                    <th class="text-center sticky">{{ $rollNo }}</th>
                                                    <td class="sticky sticky-2">{{ $studentName }}</td>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.search-result').click(function(e) {
                ...
                $('#nemc-general-form select[name="session_id"], #nemc-general-form select[name="course_id"], #nemc-general-form select[name="phase_id"]').each(function() {
                    if ($(this).val() === '') {
                        valid = false;
                        emptyFields.push($(this).attr('name').replace('_id', ''));
                    }
                });

                if (!valid) {
                    sweetAlert('Please select: ' + emptyFields.join(', '), 'error');
                    return false;
                }
