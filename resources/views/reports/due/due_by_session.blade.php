        ...
        <div class="col-md-12">
            <div class="m-portlet">
                ...
                <div class="m-portlet__body">
                    <div class="m-section__content mb-4">
                        <form id="searchForm" role="form" method="get">
                            <div class="row">
                                <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3 col-xl-3">
                                    <div class="form-group">
                                        <select class="form-control m_bootstrp m_selectpicker" name="session_id"
                                                id="session_id" data-live-search="true" required>
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                    <div class="form-group">
                                        <select class="form-control m_selectpicker" name="course_id" id="course_id">
                                            ...
                                            {!! select($courses, Auth::user()->teacher->course_id ?? Auth::user()->adminUser->course_id ?? app()->request->course_id) !!}
                                        </select>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-12 col-lg-12 col-xl-12">
                                    <div class="pull-right search-action-btn">
                                            ...
                                            <div class="btn-group" data-hover="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right w-100">
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.due.session.pdf', array_merge(request()->all(), ['page_layout' => 'A4-landscape'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-h"></i> Landscape
                                                    </a>
                                                    <a class="dropdown-item"
                                                       href="{{ route('report.due.session.pdf', array_merge(request()->all(), ['page_layout' => 'A4-portrait'])) }}"
                                                       target="_blank">
                                                        <i class="fas fa-arrows-alt-v"></i> Portrait
                                                    </a>
                                                </div>
                                            </div>
                                            <a href="{{route('report.due.session.excel', [
                                        ...
                                        <a class="btn btn-default m-btn m-btn--icon"
                                           href="{{url('admin/report_exam_result')}}">
                    ...
                    @if(!empty($studentsDue))
                        ...
                        <div class="row">
                            <div class="col-md-12 no-padding">
                                <h4 class="text-center text-decoration-underline mb-4">Due Summary</h4>
                                <div class="row">
                                    @php
                                        $currencyGroups = collect($studentsDue)->groupBy('currency')->sortKeysDesc();
                                    @endphp
                                            ...
                                            <div class="card border-info">
                                                ...
                                                <div class="card-body p-3">
                                                    <table class="table table-sm table-bordered mb-0">
                                                        <tr>
                                                            <th>Total Payable</th>
                                                            <td class="text-right">{!! $currency !!} {{ formatAmount($payable) }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th>Total Discount</th>
                                                            <td class="text-right">{!! $currency !!} {{ formatAmount($discount) }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th>Total Paid</th>
                                                            <td class="text-right">{!! $currency !!} {{ formatAmount($paid) }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th class="text-danger font-weight-bold">Total
                                                                Due
                                                            </th>
                                                            <td class="text-right text-danger font-weight-bold">{!! $currency !!} {{ formatAmount($due) }}</td>
                                                        </tr>
                                                    </table>
                                                </div>
                                            </div>
                        ...
                        <div class="row">
                            <div class="table table-responsive">
                                <h4 class="text-center mb-4">Due Details</h4>
                                <table class="table table-bordered sticky-header">
                                    ...
                                    @foreach($studentsDue as $student)
                                        <tr>
                                            ...
                                            <td class="text-right">{{ $student['currency'] .' '. formatAmount($student['total_payable']) }}</td>
                                            <td class="text-right">{{ $student['currency'] .' '. formatAmount($student['total_discount']) }}</td>
                                            <td class="text-right">{{ $student['currency'] .' '. formatAmount($student['total_paid']) }}</td>
                                            <td class="text-right text-danger">{{ $student['currency'] .' '. formatAmount($student['total_due']) }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
