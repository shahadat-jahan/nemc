    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <div class="table-responsive">
            <table class="table table-bordered" id="attachment-table">
                ...
                <tbody>
                @if($applicant->admissionAttachments)
                    @foreach($applicant->admissionAttachments as $attachment)
                <tr>
                    <td><input type='hidden' name='attachment_type_id[]' value='{{$attachment->attachment_type_id}}'/>{{$attachment->attachmentType->title}}</td>
                    <td><input type='hidden' name='remarks[]' value='{{$attachment->remarks}}'/>{{$attachment->remarks}}</td>
                    <td><input type='hidden' name='file_path[]' value='{{$attachment->file_path}}'/><input type='hidden' name='type[]' value='{{$attachment->type}}'/>
                        <a href='{{asset($attachment->file_path)}}' target='_blank'>View</a></td>
                    <td><a href='#' class='remove-attachment' title='Remove attachment'><i class='fa fa-trash-alt'></i></a></td>
                    </tr>
                    @endforeach
                    @endif
                </tbody>
...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
        ...
        <?php
        $photo = !empty($applicant) ? $applicant->photo : 'assets/global/img/import_placeholder.png';
        ?>
        <img id="user-image" class="img-fluid" src="{{asset($photo)}}" alt="">
        <input type="hidden" name="applicant_photo" value="{{$applicant->photo}}" id="user-profile-pic">
    </div>
</div>
