...
@section('content')
    <div class="m-portlet m-portlet--tab">
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Student Group</h3>
                </div>
            </div>
        ...
        <form class="m-form m-form--fit m-form--label-align-right"
              action="{{ route('student_group_type.update', [$studentGroupType->id]) }}" method="post"
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('title') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="title"
                                   value="{{old('title', $studentGroupType->title) }}"
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('class_type_ids') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m_selectpicker"
                                ...
                                {!! select($classTypes, $studentGroupType->classTypes->map(fn($classType)=> $classType->id)->toArray()) !!}
                            </select>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div
                            ...
                            <select class="form-control m_selectpicker" name="exam_category_ids[]" multiple>
                                ...
                                @foreach($examCategories as $id => $examCategory)
                                    <option value="{{ $id }}"
                                        {{ in_array($id, old('exam_category_ids', $studentGroupType->ExamCategories->map(fn($examCategory)=>$examCategory->id)->toArray())) ? 'selected' : '' }}>
                                        {{ $examCategory}}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <textarea class="form-control m-input" name="description"
                                      placeholder="Description">{{ old('description', $studentGroupType->description) }}</textarea>
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <select class="form-control m_selectpicker" name="status">
                                <option value="1" {{ $studentGroupType->status == 1  ? 'selected' : '' }}>Active
                                </option>
                                <option value="0" {{ $studentGroupType->status == 0  ? 'selected' : '' }}>Inactive
                                </option>
                            </select>
                        </div>
                    </div>
