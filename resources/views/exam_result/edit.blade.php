    ...
    <div class="m-portlet m-portlet--tab">
        ...
        <div class="m-portlet__body">
            <div class="m-section__content">
                <div class="card">
                    <div class="card-header">Common Information</div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12">
                                <div class="">
                ...
                <br>
                <div class="card">
                    <div class="card-header">Edit Student Exam Result</div>
                    <div class="card-body">
                        <form class="m-form m-form--fit m-form--label-align-right"
                              action="{{ route('exam.subject.result.update', [$examInfo->id, $subject->id]) }}"
                            ...
                            @if($authUser->adminUser ||  $authUser->user_group_id == 12)
                                <div class="row {{$isPublish == 1 ? 'd-none' : ''}}">
                                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                                        <div
                                            class="form-group  m-form__group {{ $errors->has('subject_id') ? 'has-danger' : '' }}">
                                            <div class="m-checkbox-inline">
                                                <label class="m-checkbox">
                                                    <input type="checkbox" name="publish_result"
                                                           value="1"> Publish
                                                    result for this subject?
                                                    <span></span>
                                                </label>
                                            </div>
                                            <small class="text-danger">Once published, result is no longer
                                                editable.</small>
                                    ...
                                    <table class="table table-bordered table-hover sticky-header" id="exam-results">
                                        ...
                                        <tr>
                                            ...
                                            @foreach($examTypeSubType as $type)
                                                @php
                                                    $examTypeMarks = $type->examSubTypes->sum(fn($subType) => $subType->examSubjectMark->sum('total_marks'));
                                                    $examTotalMarks += $examTypeMarks;
                                                    $examTypeMarksArr[$type->id] = $examTypeMarks;
                                                @endphp
                                                <th colspan="{{ count($type->examSubTypes) }}" class="align-middle">{{ $type->title }} ({{ $examTypeMarks }})</th>
                                            @endforeach
                                            ...
                                            @foreach($examTypeSubType as $type)
                                                @foreach($type->examSubTypes as $subType)
                                                    @foreach($subType->examSubjectMark as $mark)
                                                        @php $examSubjectMarkIds[] = $mark->id; @endphp
                                                        <th class="align-middle">{{ $subType->title }} ({{ $mark->total_marks }})</th>
                                                    @endforeach
                                                @endforeach
                                            @endforeach
                                        </tr>
                                        ...
                                        @foreach($examResult->groupBy('student_id') as $studentId => $studentResult)
                                            @php
                                                $student = $studentResult->first()->student;
                                                $totalMarksByExamType = [];
                                                $totalGrace = 0;
                                                $failCount = 0;
                                                $passCount = 0;
                                                $special = $studentResult->whereIn('exam_subject_mark_id', $examSubjectMarkIds)->whereNotNull('special_status')->count();
                                                $absent = $studentResult->whereIn('exam_subject_mark_id', $examSubjectMarkIds)->where('pass_status', 4)->count();
                                                $grace = $studentResult->whereIn('exam_subject_mark_id', $examSubjectMarkIds)->whereNotNull('grace_marks')->count();
                                            @endphp
                                            <tr>
                                                <td class="text-center align-middle">{{ $student->roll_no }}
                                                    <input type="hidden" name="student_id[{{ $studentId }}]" value="{{ $studentId }}"/>
                                                </td>
                                                ...
                                                @foreach($studentResult->sortBy('id') as $result)
                                                    @php
                                                        $subjectMark = $result->examSubjectMark;
                                                        $examTypeId = $subjectMark->examSubType->examType->id;
                                                        $marks = $result->marks;
                                                        $graceMarks = $result->grace_marks;
                                                        $totalMark = $marks + $graceMarks;
                                                        $totalGrace += $graceMarks;
                                                        $totalMarksByExamType[$examTypeId] = ($totalMarksByExamType[$examTypeId] ?? 0) + $totalMark;
                                                    @endphp
                                                    <td>
                                                        <input type="number" class="form-control m-input sub_type_mark" min="0" max="{{ $subjectMark->total_marks }}" name="sub_type_mark[{{ $studentId }}][{{ $subjectMark->id }}]" value="{{ $result->pass_status === 4 ? '' : $totalMark }}" autocomplete="off"/>
                                                    </td>
                                                @endforeach
                                                @php
                                                    ...
                                                    if ($special > 0) {
                                                        $isSpecialPass = $studentResult->where('special_status', 1)->count() > 0;
                                                ...
                                                @endphp
                                                <td class="text-center align-middle"><span class="{{ $colorClass }}">{{ $resultStatus }}</span></td>
                                                <td>
                                                    <input type="text" class="form-control m-input w-100" name="comment[{{ $studentId }}]" value="{{ $result->remarks }}" autocomplete="off"/>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                            ...
                            <div class="m-form__actions text-center">
                                <div class="m-form__actions">
                                    <a href="{{ route('result.index') }}" class="btn btn-outline-brand"><i
