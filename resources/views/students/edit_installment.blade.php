...
@section('content')
    <div class="m-portlet m-portlet--tab">
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Installments of {{$student->full_name_en}}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="row installment-row plan-row m--hide">
            ...
            <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                <div class="form-group  m-form__group">
                    ...
                    <input type="text" class="form-control m-input last-date-payment" name="last_date_of_payment[]" placeholder="Last date of payment" autocomplete="off"/>
                </div>
            </div>
        ...
        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right"  action="{{ route('students.installment.update', $student->id) }}" id="nemc-general-form" method="post">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('total_development_fee') ? 'has-danger' : '' }}">
                            ...
                            <?php
                                $total_amount=$installments[0]->fee->total_amount;
                                $total_development_fee=!empty($total_amount) ? $total_amount : $developmentFee;
                            ?>
...
{{--                            <input type="text" class="form-control m-input" name="total_development_fee" value="{{formatAmount($developmentFee)}}" placeholder="Total Development Fee"/>--}}
                            <input type="text" class="form-control m-input" name="total_development_fee" value="{{formatAmount($total_development_fee)}}" placeholder="Total Development Fee" readonly/>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('total_payable_amount') ? 'has-danger' : '' }}">
                            ...
                            <input type="hidden" id="total_payable_amount" value="{{$installments[0]->fee->payable_amount}}" placeholder="Total Development Fee" required/>
                            <input type="text" class="form-control m-input" name="total_payable_amount" max="{{formatAmount($total_development_fee)}}" id="payable_amount" value="{{formatAmount($installments[0]->fee->payable_amount)}}" placeholder="Total Development Fee" readonly/>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('total_installment') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input" name="total_installment" id="total_installment">
                                ...
                                @for ($i = 1; $i <= 20; $i++)
                                    <?php $selected = ($i == count($installments)) ? 'selected' : '' ?>
                ...
                <div id="all-installments">
                    @foreach($installments as $key => $installment)
                        <div class="row plan-row">
                            <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                <div class="form-group  m-form__group">
                                    <span class="installment-title">Installment {{$key + 1}}</span>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                <div class="form-group  m-form__group">
                                    <label class="form-control-label"><span class="text-danger">*</span>  Amount </label>
                                    <input type="number" class="form-control m-input installment-amount" name="amount[]" min="1" value="{{$installment->payable_amount}}" placeholder="Amount"/>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                                <div class="form-group  m-form__group">
                                    <label class="form-control-label"> Last date of payment </label>
                                    <input type="text" class="form-control m-input last-date-payment m_datepicker_1" name="last_date_of_payment[]" value="{{$installment->last_date_of_payment}}" placeholder="Last date of payment" autocomplete="off"/>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-1 col-lg-1 col-xl-1">
                                <a href="#" class="btn btn-sm btn-danger mt-5 remove-row"><i class="fa fa-trash-alt"></i></a>
        ...
        $(document).ready(function() {
            ...
            function setupInstallments(totalInstallment){
                for (var i=1; i<=totalInstallment; i++){
                    ...
                    installmentNumber = (parseInt($('#all-installments > .row').length + 1));
                    installmentRow.find('.installment-title').text('Installment '+ installmentNumber );
                    //installmentRow.find('.installment-title').text('Installment '+ (parseInt($('#all-installments > .row').length) + i ));
                    installmentRow.find('.last-date-payment').datepicker( {
                        todayHighlight: !0,
                        orientation: "bottom left",
                        format: 'dd/mm/yyyy',
                    });
            ...
            $('#total_installment').change(function () {
                ...
                currentInstallments = $('#all-installments > .row').length;
                chnagedStatus = installments - currentInstallments;
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            ...
                            if (chnagedStatus < 0){
                                $('#all-installments').find('.plan-row:gt('+Math.abs(chnagedStatus - 1)+')').remove();
                            } else{
                                setupInstallments(chnagedStatus);
                            }
            ...
            $('#payable_amount').keypress(_.debounce(function(){
