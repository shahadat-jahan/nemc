            ...
            <div class="m-portlet view-page">
                <div class="m-portlet__head">
                    ...
                    <div class="m-portlet__head-tools">
                        <a href="{{ route('exams.list') }}" class="btn btn-primary m-btn m-btn--icon" title="Applicants"><i class="fas fa-archway"></i> Exams</a>
                    </div>
                </div>
                                            ...
                                            <ul class="list-group list-group-flush">
                                                ...
                                                <li class="list-group-item">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-8">{{$examInfo->title}}</div>
                                                    </div>
                                                </li>
                                            </ul>
                                        </div>
                                                            ...
                                                            @if(!empty($wExam->examSubTypes->toArray()))
                                                                @foreach($wExam->examSubTypes as $subType)
                                                                    <tr>
                                                                        ...
                                                                        <td>{{$wExam->subject->title}}</td>
                                                                        <td>{{$subType->title}}</td>
                                                                        <td>{{!empty($subType->pivot->exam_time) ? parseClassTimeInTwelveHours($subType->pivot->exam_time) :'-'}}</td>
                                                                    </tr>
                                                                @endforeach
                                                            @else
                                                            <tr>
                                                                <td>{{$wExam->exam_date}}</td>
                                                                {{--<td>{{date('d-m-Y'), strtotime($wExam->exam_date)}}</td>--}}
                                                                <td>{{$wExam->subject->title}}</td>
                                                ...
                                                <table class="table table-bordered table-hover" id="dataTable">
                                                    ...
                                                    <tbody>
                                                        @foreach($practicalExams as $pExam)
                                                            @if(!empty($pExam->examSubTypes->toArray()))
                                                                @foreach($pExam->examSubTypes as $key => $subType)
                                                                    <tr>
                                                                        <td>{{$subType->pivot->exam_date}}</td>
                                                                        <td>{{$pExam->subject->title}}</td>
                                                                        <td>{{isset($pExam->examSubTypes[$key]) ? $pExam->examSubTypes[$key]->title : ''}}</td>
                                                                        <td>{{isset($pExam->studentGroups[$key]) ? $pExam->studentGroups[$key]->group_name : 'All'}}</td>
                                                                        <td>{{!empty($subType->pivot->exam_time) ? parseClassTimeInTwelveHours($subType->pivot->exam_time) : '-'}}</td>
                                                                    </tr>
                                                                @endforeach
                                                            @else
                                                            <tr>
                                                                ...
                                                                <td>-</td>
                                                                <td>All</td>
                                                                <td>{{!empty($pExam->exam_time) ? parseClassTimeInTwelveHours($pExam->exam_time) : '-'}}</td>
                                                            </tr>
                                                            @endif
                                                        @endforeach
                                                    </tbody>
