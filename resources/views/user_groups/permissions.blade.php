    ...
    <div class="container-fluid">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="mb-0">Update User Group Permissions</h3>
                <a href="{{ url('admin/user_groups') }}" class="btn btn-primary"><i class="fa fa-users pr-2"></i>User Group</a>
            </div>
            <form action="{{ url('admin/user_groups/' . $userGroup->id . '/permission') }}" method="post">
                @csrf
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-6">
                    ...
                    <div class="row">
                        @foreach (Config::get('menu') as $key => $val)
                            @php
                                $checked =
                                    (array_key_exists($key, $userGroupPermissions) and $userGroupPermissions[$key] > 0)
                                        ? 'checked'
                                        : '';
                            @endphp
                                ...
                                <div class="card h-100 border-info">
                                    <div class="card-header bg-info text-white py-2">
                                        <div class="form-check">
                                            <input class="form-check-input" style="transform:scale(1.3);margin-right:10px;"
                                                type="checkbox" name="action[]" value="{{ $key }}"
                                                id="perm-{{ $key }}" {{ $checked }}>
                                                                ...
                                                                @foreach ($cVal['actions'] as $aKey => $aVal)
                                                                    @php
                                                                        $cAChecked =
                                                                            ...
                                                                            $userGroupPermissions[$cKey . '/' . $aVal] >
                                                                                0)
                                                                                ? 'checked'
                                                                                : '';
                                                                    @endphp
    ...
    <script>
        ...
        $('input[type="checkbox"]').change(function() {
            ...
            if ($this.closest('.card-header').length) {
                // Module checkbox
                $this.closest('.card').find('input[type="checkbox"]').prop('checked', this.checked);
            } else if ($this.closest('.bg-light').length && !$this.closest('.row').is('.ms-3')) {
                // Child checkbox
                var childBlock = $this.closest('.bg-light');
                childBlock.find('.row.ms-3 input[type="checkbox"]').prop('checked', this.checked);
            } else if ($this.closest('.ms-2').length) {
                // (Optional) If you want to update module checkbox based on all children
                var card = $this.closest('.card');
                var allChecked = card.find('.ms-2 input[type="checkbox"]').length === card.find(
                    '.ms-2 input[type="checkbox"]:checked').length;
                card.find('.card-header input[type="checkbox"]').prop('checked', allChecked);
            }

            // ACTION: If any action under a child is checked, check the child; if none, uncheck the child
            if ($this.closest('.row.ms-3').length) {
                var childBlock = $this.closest('.bg-light');
                var actions = childBlock.find('.row.ms-3 input[type="checkbox"]');
                var checkedActions = actions.filter(':checked').length;
                var childCheckbox = childBlock.find('> .form-check input[type="checkbox"]');
            ...
            if (card.length) {
                ...
                var moduleCheckbox = card.find('.card-header input[type="checkbox"]');
