        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Payment Detail</h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
                    <a href="{{ url('admin/payment_detail') }}" class="btn btn-primary m-btn m-btn--icon"><i class="fas fa-list pr-2"></i>Payment Details</a>
            </div>
        </div>

        <form class="m-form m-form--fit m-form--label-align-right"  action="{{ url('admin/payment_detail/' .$paymentDetail->id) }}" method="post">
            @csrf
            @method('PUT')
                        ...
                        <div class="form-group  m-form__group {{ $errors->has('payment_type_id') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input m-bootstrap-select m_selectpicker" name="payment_type_id" id="payment_type_id" data-live-search="true">
                                ...
                                @foreach($paymentTypes as $paymentType)
                                    <option value="{{$paymentType->id}}" {{$paymentType->id == $paymentDetail->paymentType->id ? 'selected' : ' '}}>{{$paymentType->title}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group  m-form__group {{ $errors->has('student_category_id') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input m-bootstrap-select m_selectpicker" name="student_category_id" id="student_category_id" data-live-search="true">
                                ...
                                @foreach($studentCategories as $studentCategory)
                                    <option value="{{$studentCategory->id}}" {{$studentCategory->id == $paymentDetail->studentCategory->id ? 'selected' : ' '}}>{{$studentCategory->title}}</option>
                                @endforeach
                            </select>
                ...
                <div class="row">
                        ...
                        <div class="form-group  m-form__group {{ $errors->has('session_id') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input m-bootstrap-select m_selectpicker" name="session_id" data-live-search="true">
                                ...
                                @foreach($sessions as $session)
                                    <option value="{{$session->id}}" {{$session->id == $paymentDetail->session_id ? 'selected' : ' '}}>{{$session->title}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group  m-form__group {{ $errors->has('course_id') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input m-bootstrap-select m_selectpicker" name="course_id" id="course_id" data-live-search="true">
                                ...
                                @foreach($courses as $course)
                                    <option value="{{$course->id}}" {{$course->id == $paymentDetail->course->id ? 'selected' : ' '}}>{{$course->title}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('amount') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="amount" value="{{ old('amount', $paymentDetail->amount) }}" placeholder="Enter amount"/>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('currency_code') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input" name="currency_code">
                                <option value="TK" {{ $paymentDetail->currency_code == 'TK'  ? 'selected' : '' }}>TK</option>
                                <option value="USD" {{ $paymentDetail->currency_code == 'USD'  ? 'selected' : '' }}>USD</option>
                            </select>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <select class="form-control m-input " name="status">
                                <option value="1" {{ $paymentDetail->status == 1  ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ $paymentDetail->status == 0  ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                    </div>
                </div>
