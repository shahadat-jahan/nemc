    ...
    <div class="m-portlet m-portlet--tab">
        <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
                <a href="{{ route('frontend.message.list') }}" class="btn btn-primary m-btn m-btn--icon"><i class="fas fa-list-alt pr-2"></i>Messages</a>
            </div>
        </div>

        <div class="m-form m-form--fit m-form--label-align-right">
            <div class="m-portlet__body">
                @if(in_array(Auth::guard('student_parent')->user()->id, [$userMessage->created_by, $userMessage->user_id]))
                <div class="col-sm-12">
                    @php
                        $messageBy='';
                    ...
                    <h6><i class="far fa-clock"></i> Message by {{$messageBy}}, {{$userMessage->created_at->format('F j, Y, g:i A')}}</h6>
                    <p>Message : {{strip_tags($userMessage->message)}}</p>
                    <h5>
                        ...
                        <span class="badge badge-info">Reply: {{$userMessage->is_replied == 0 ? 'Don not reply'  : 'Replied'}}</span>
                    </h5>
                    @if(!empty($userMessage->file_path))
                        <div>
                            <ul class="list-inline">
                                <li class="list-inline-item">Attachment File : </li>
                                <li class="list-inline-item"><a class="" target="_blank" href="{{asset('nemc_files/message/'.$userMessage->file_path)}}" download title="Download attachment file"> <i class="fas fa-download"></i></a></li>
                            </ul>
                        </div>
                    @endif
                    <hr>
                        ...
                        <div class="media">
                            @if(isset($messageReply->createdBy->adminUser))
                                @if(!empty($messageReply->createdBy->adminUser->photo))
                                    <img src="{{asset($messageReply->createdBy->adminUser->photo)}}" alt="Admin image" width ="70">
                                @else
                                    <img src="{{asset('assets/global/img/male_avater.png')}}" alt="User image" width="70">
                                @endif
                            @elseif(isset($messageReply->createdBy->student))
                                @if(!empty($messageReply->createdBy->student->photo))
                                    <img src="{{asset($messageReply->createdBy->student->photo)}}" alt="Admin image" width ="70">
                                @else
                                    <img src="{{asset('assets/global/img/male_avater.png')}}" alt="User image" width="70">
                                @endif
                            ...
                            @else
                                @if(!empty($messageReply->createdBy->teacher->photo))
                                    <img src="{{asset($messageReply->createdBy->teacher->photo)}}" alt="Admin image" width ="70">
                                @else
                                    <img src="{{asset('assets/global/img/male_avater.png')}}" alt="User image" width="70">
                                @endif
                            @endif
                    ...
                    <h4>Leave a Reply:</h4>
                    <p>Total <span class="badge badge-info">{{$userMessage->messageReplies->count()}}</span> replies</p><br>

                    <form role="form" action="{{route('frontend.message.reply', [$userMessage->id])}}" method="post" id="replyForm">
                        @csrf
                        ...
                        <div class="text-right">
                            <button type="submit" class="btn btn-success not-allowed" disabled><i class="fas fa-reply"></i> Reply</button>
                        </div>
                    </form>
                    <br><br>
                </div>
