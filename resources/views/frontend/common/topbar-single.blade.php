                                            ...
                                            <div class="m-card-user m-card-user--skin-light">
                                                ...
                                                @if(Auth::guard('student_parent')->check())
                                                    ...
                                                    <span class="m-card-user__name m--font-weight-500">
                                                        @if(Auth::guard('student_parent')->user()->user_group_id == 5)
                                                            {{Auth::guard('student_parent')->user()->student->full_name_en}}
                                                        @else
                                                            {{!empty(Auth::guard('student_parent')->user()->parent->father_name)? Auth::guard('student_parent')->user()->parent->father_name: Auth::guard('student_parent')->user()->parent->mother_name}}
                                                        @endif
                                                    </span>
                                                            <a href="" class="m-card-user__email m--font-weight-300 m-link">
                                                                @if(Auth::guard('student_parent')->user()->user_group_id == 5)
                                                                    {{Auth::guard('student_parent')->user()->student->email}}
                                                                @else
                                                                    {{Auth::guard('student_parent')->user()->parent->father_email}}
                                                                @endif
                                                            </a>
                                                        </div>
                                                    </div>
