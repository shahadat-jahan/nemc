                            ...
                            <div class="row mt-4">
                                <div class="col-xs-12 col-sm-6 col-md-12 col-lg-12 col-xl-12">
                                    <div class="pull-right search-action-btn">
                                        @if(!empty($examResult))
                                            <a target="_blank" href="{{route(customRoute('report.exam_result.student.pdf'), [
                                        ...
                                        'subject_id' => app()->request->subject_id,
                                                ...
                                                PDF</a>
                                            <a href="{{route(customRoute('report.exam_result.student.excel'), [
                                        'session_id' => app()->request->session_id,
                                        'course_id' => app()->request->course_id,
                                        'phase_id' => app()->request->phase_id,
                                        'term_id' => app()->request->term_id,
                                        ...
                                        <a class="btn btn-default m-btn m-btn--icon"
                                           href="{{url('nemc/report_exam_result_student')}}">
    ...
    <script>
        ...
        $('#session_id, #course_id, #phase_id, #term_id, #exam_category_id').change(function () {
            ...
            if (sessionId > 0 && courseId > 0 && phaseId > 0 && termId > 0 && examCategoryId > 0) {
                ...
                $.get('{{route(customRoute('exam.list.session.course.phase.term.type'))}}', {
                    sessionId: sessionId,
                    courseId: courseId,
                    phaseId: phaseId,
                    termId: termId,
        ...
        $('#exam_id').change(function () {
            if ($(this).val() > 0) {
                ...
                $.get('{{route(customRoute('subjects.list.examId'))}}', {examId: $(this).val()}, function (response) {
        ...
        if (examId > 0) {
            ...
            $.get('{{route(customRoute('subjects.list.examId'))}}', {examId: examId}, function (response) {
                if (response.subjects) {
                    $('#subject_id').html('<option value="">---- Select Subject ----</option>');
                    for (i in response.subjects) {
                        subject = response.subjects[i];
