        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="fas fa-edit pr-2"></i>Update Teacher Evaluation</h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
                <a href="{{ route('frontend.teacher.show', $teacher->id) }}"
                   class="btn btn-primary m-btn m-btn--icon"><i class="fas fa-user-tie pr-2"></i>Back to Teacher</a>
            </div>
        </div>

        <form action="{{ route('frontend.teacher.evaluation.update', $evaluation->id) }}" method="POST"
              id="evaluationForm">
            @csrf
            @method('PUT')
                                    ...
                                    <td style="text-align: center;">
                                        <label class="m-radio">
                                            <input type="radio" name="statement_{{ $id }}_rating"
                                                ...
                                                {{ (old("statement_{$id}_rating") ?? $ratings[$id] ?? null)
                                                    == $rating ? 'checked' : '' }}>
                                            <span></span>
                                        </label>
                                    </td>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <div class="m-radio-inline">
                                <label class="m-radio">
                                    <input type="radio" name="is_role_model"
                                           value="1" {{ $evaluation->is_role_model == 1 ? 'checked' : '' }}>
                                    Yes
                                    <span></span>
                                </label>
                                <label class="m-radio">
                                    <input type="radio" name="is_role_model"
                                           value="0" {{ $evaluation->is_role_model == 0 ? 'checked' : '' }}>
                                    No
                                    <span></span>
                                </label>
                            </div>
