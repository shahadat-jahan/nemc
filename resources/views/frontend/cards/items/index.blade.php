                                    ...
                                    <div class="form-group">
                                        <select class="form-control" name="course_id" id="course_id">
                                            ...
                                            @foreach($courses as $course)
                                                <option value="{{$course->id}}" {{$course->id == $courseId ? 'selected' : ''}}>{{$course->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
<script>
    ...
    $('#subject_id').change(function (e) {
        ...
        $.get(baseUrl+'nemc/cards/subjects/'+subjectChoiseId,{},function (response) {
    ...
    if (cardId > 0 && subjectId > 0 && courseId > 0 && subjectGroupId > 0){
        var ajax1 = $.get('{{route(customRoute('subjectGroup.course'))}}', {courseId: courseId}, function (response) {
                if (response.data){
                    $('#subject_group_id').html('<option value="">---- Select ----</option>');
                    for (i in response.data){
                        subjectGroup = response.data[i];
            ...
            ajax2 = $.get('{{route(customRoute('subjects.course.group'))}}', {courseId: courseId, subjectGroupId: subjectGroupId}, function (response) {
                if (response.data){
                    $('#subject_id').html('<option value="">---- Select ----</option>');
                    for (i in response.data){
                        subject = response.data[i];
            ...
            ajax3 = $.get(baseUrl+'nemc/cards/subjects/'+subjectId,{},function (response) {
                $("#card_id").html('<option value="">--Select Card--</option>');
                if (response.data.length > 0){
                    for (i in response.data){
                        item = response.data[i];
