        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Book</h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
                <a href="{{ url('admin/book') }}" class="btn btn-primary m-btn m-btn--icon"><i
                        class="fas fa-book-open pr-2"></i>Books</a>
            </div>
        </div>

        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right" id="nemc-general-form"
              action="{{ url('admin/book/' .$book->id) }}" method="post">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('title') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="title"
                                   value="{{ old('title', $book->title) }}" placeholder="Book Name"/>
                            @if ($errors->has('title'))
                        ...
                        <div class="form-group  m-form__group {{ $errors->has('course_id') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control" name="course_id" id="course_id">
                                ...
                                @foreach($courses as $course)
                                    <option
                                        value="{{$course->id}}" {{($course->id == $book->subject->subjectGroup->course_id) ? 'selected' : ''}}>{{$course->title}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('author') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="author"
                                   value="{{ old('author', $book->author) }}" placeholder="Author Name"/>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <input type="text" class="form-control m-input" name="edition"
                                   value="{{ old('edition', $book->edition) }}" placeholder="Book Edition"/>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <input type="text" class="form-control m-input" name="link_title"
                                   value="{{ old('link_title', $book->link_title) }}" placeholder="Link title"/>
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group">
                            ...
                            <input type="text" class="form-control m-input" name="link"
                                   value="{{ old('link', $book->link) }}" placeholder="Link"/>
                        </div>
                    </div>
                </div>
                <div class="row">
            ...
            <div class="m-portlet__foot m-portlet__foot--fit">
                <div class="m-form__actions text-center">
                    <a href="{{ url('admin/book') }}" class="btn btn-outline-brand"><i class="fa fa-times"></i>
                        Cancel</a>
...
@push('scripts')
    ...
    <script>
        var subjectGroupId = '{{$book->subject->subject_group_id}}';
        var subjectId = '{{$book->subject_id}}';
        ...
        if (subjectGroupId > 0 && subjectId > 0) {
            ...
            var ajax1 = $.get('{{route('SubjectGroup.course')}}', {courseId: $('#course_id').val()}, function (response) {
                ...
                ajax2 = $.get('{{route('subjects.course.group')}}', {
