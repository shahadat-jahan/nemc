        ...
        <div class="col-md-12">
            <div class="m-portlet">
                <div class="m-portlet__head">
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text"><i class="fas fa-list-ul pr-2"></i>Academic Calender List
                            </h3>
                        </div>
                    </div>
                ...
                <div class="m-portlet__body">
                    <form role="form" method="get" action="{{route('get.academic.calender.data')}}">
                        <div class="form-body">
                            <div class="row">
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-4">
                                    <div class="form-group">
                                        <select class="form-control m-input" name="subject_group_id"
                                                id="subject_group_id">
                                ...
                                <div class="col-md-12">
                                    <div class="pull-right search-action-btn">
                                        <button class="btn btn-primary m-btn m-btn--icon"><i class="fa fa-plus"></i>
                                            Generate
                                        </button>
                                        <a target="_blank" href="{{route('academic.calender.data.pdf', [
                                        'session_id' => app()->request->session_id, 'course_id' => app()->request->course_id, 'subject_group_id' => app()->request->subject_group_id
                                        ])}}" class="btn btn-info m-btn m-btn--icon"><i
                                            ...
                                            @if($holidays->isNotEmpty())
                                                @foreach($holidays as $holiday)
                                                    <tr>
                                                        ...
                                                        <td>{{$holiday->title}}</td>
                                                        <td>{{!empty($holiday->from_date) ? fullDateFormat($holiday->from_date) : 'N/A'}}</td>
                                                        <td>{{!empty($holiday->to_date)? fulldateFormat($holiday->to_date) : 'N/A'}}</td>
                                                    </tr>
                                                @endforeach
                                            @else
                                                <tr>
                                            ...
                                            @if($classRoutines->isNotEmpty())
                                                @foreach($classRoutines as $classRoutine)
                                                    <tr>
                                                        ...
                                                        <td>{{date("d/m/Y", strtotime($classRoutine->class_date))}}</td>
                                                        ...
                                                        <td>
                                                            <a href="{{route(customRoute('class_routine.info.single'), $classRoutine->id)}}"
                                                               target="_blank"
                                                               class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"
                                                               title="View, Attendance, Lecture Material"><i
...
@push('scripts')
    <script>
        ...
        $('#session_id, #course_id').change(function (e) {
            ...
            // load subject group
            $.get('{{route('subjects.session.course')}}', {
                sessionId: sessionId,
                courseId: courseId
            }, function (response) {
                if (response.data) {
