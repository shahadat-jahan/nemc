...
@section('content')
    <div class="m-portlet m-portlet--tab">
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="far fa-edit pr-2"></i>Edit Exam Category</h3>
                </div>
            </div>
        ...
        <!--begin::Form-->
        <form class="m-form m-form--fit m-form--label-align-right"  action="{{ url('admin/exam_category/' .$examCategory->id) }}" method="post">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('title') ? 'has-danger' : '' }}">
                            ...
                            <input type="text" class="form-control m-input" name="title" value="{{ old('title', $examCategory->title) }}" placeholder="Exam Category Title"/>
                            @if ($errors->has('title'))
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                        <div class="form-group  m-form__group {{ $errors->has('status') ? 'has-danger' : '' }}">
                            ...
                            <select class="form-control m-input " name="status">
                                <option value="1" {{ $examCategory->status == 1  ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ $examCategory->status == 0  ? 'selected' : '' }}>Inactive</option>
                            </select>
