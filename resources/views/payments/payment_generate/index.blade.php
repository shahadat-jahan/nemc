                ...
                <div class="m-portlet__head">
                    ...
                    <div class="m-portlet__head-tools">
                        @if (hasPermission('collect_fee/create'))
                            <a href="{{ route('student.absent.fee.create') }}"
                               class="btn btn-primary m-btn m-btn--icon"><i class="fa fa-plus"></i> Generate Class
                                Absent Fee</a>
                        @endif
                    </div>
                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($authUser->adminUser->course_id == 1)
                                                <select class="form-control" name="course_id" id="course_id">
                                                    ...
                                                    <option
                                                        value="1" {{ $authUser->adminUser->course_id == 1 ? 'selected' : ''}}>
                                                        MBBS
                                                    </option>
                                                </select>
                                                <!-- Staff - Accounts (BDS)-->
                                            @elseif($authUser->adminUser->course_id == 2)
                                                <select class="form-control" name="course_id" id="course_id">
                                                    <option value="">---- Select Course ----</option>
                                                    <option
                                                        value="2" {{ $authUser->adminUser->course_id == 2 ? 'selected' : ''}}>
                                                        BDS
                                                    </option>
                                                </select>
                                ...
                                @if(!empty($studentFees))
                                    ...
                                    <div class="table m-table table-responsive">
                                        <table class="table table-bordered table-hover" id="dataTable">
                                            ...
                                            @endphp
                                            @foreach($sortedStudentFees as $key=>$studentFee)
                                                    <?php
                                                    $payment_type_id = $studentFee->feeDetails->first()->payment_type_id;
                                                    $sPayments = $studentFee->studentPaymentDetails;
                                                    $disCount = [];
                                                    if (!empty($sPayments)) {
                                                    ...
                                                    $discount_amount = $studentFee->discount_amount + $discount;
                                                    $paid_amount = $studentFee->paid_amount;
                                                    $due_amount = $studentFee->due_amount - $discount;
//                                                    $discount_amount=$studentFee->studentPaymentDetails[$key]->studentPayment->discount_amount;
//                                                   $statusType=$
                                                    ?>
                                                {{--{{dd($studentFee)}}--}}
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="{{route('generate.student.fee.view', $studentFee->id)}}"
                                                           class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"
                                                           title="View"><i class="flaticon-eye"></i></a>
                                                        {{--                                                        @if($studentFee->feeDetails->first()->payment_type_id != 1)--}}
                                                        {{--                                                            <a href="{{route('generate.student.fee.edit', $studentFee->id)}}"--}}
                                                        {{--                                                               class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"--}}
