        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text"><i class="fas fa-info-circle pr-2"></i>Student Tuition Fee Details</h3>
                </div>
            </div>
                        ...
                        @if(!empty($studentFeeInfo->feeDetails))
                            ...
                            @foreach ($studentFeeInfo->feeDetails as $studentFeeDetail)
                                <tr>
                                    ...
                                    <td>{{$studentFeeDetail->total_amount}}</td>
                                    ...
                                    <td>
                                        @if(!empty($studentFeeDetail->discount_application))
                                            <a href="{{asset($studentFeeDetail->discount_application)}}" title="Download Discount Application" download><i class="fas fa-download"></i></a>
                                        @else
                                            <span>--</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if($studentFeeDetail->payment_type_id == 1)
                                            <a href="{{route('development.installment-fee.single.edit', $studentFeeDetail->id)}}" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Edit"><i class="flaticon-edit"></i></a>
                                       @else
                                            <span>--</span>
                                        @endif
                                    </td>
